...
import type { ExecutionResult, Language, SubmissionRecord, TestCase, TestRunResponse } from "@shared/api";
...
export default function Index() {
  ...
  const submit = async () => {
    ...
    const rec: SubmissionRecord = {
      ...
      runSummary: testResults ? { total: testResults.total, passed: testResults.passed, durationMs: testResults.results.reduce((a, b) => a + b.durationMs, 0) } : result ? { total: 1, passed: result.exitCode === 0 && !result.timedOut ? 1 : 0, durationMs: result.durationMs } : null,
      lastRun: result || null,
    };
    ...
    <Layout>
      <section className="mb-6">
        <div className="rounded-xl border bg-gradient-to-br from-fuchsia-500/10 via-sky-500/10 to-emerald-500/10 p-6">
          <h1 className="text-3xl font-extrabold tracking-tight">Online Code Compiler & Judge</h1>
          <p className="text-muted-foreground mt-2 max-w-3xl">Compile and run Java or Python code against custom input or test cases. Designed to showcase DSA, backend design, scalability, and testing.</p>
        </div>
      </section>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div>
          <div className="flex flex-wrap items-center gap-3 mb-3">
            ...
            <Button variant="secondary" onClick={() => setCode(templates[language])}>Reset Template</Button>
          </div>
          <CodeEditor value={code} onChange={setCode} language={language} />
          <div className="mt-3 flex flex-wrap gap-3">
            <Button onClick={run} disabled={busy}>Run</Button>
            <Button onClick={runTests} variant="outline" disabled={busy || tests.length === 0}>Run Tests</Button>
            <Button onClick={submit} variant="secondary" disabled={busy}>Submit</Button>
          </div>
          <p className="text-xs text-muted-foreground mt-2">Note: For Java, ensure your main class is Main. Execution is sandboxed with timeouts but not fully isolated; avoid untrusted code.</p>
        </div>
        <div>
          <Tabs defaultValue="input">
            ...
            <TabsContent value="input" className="mt-3">
              <Textarea value={stdin} onChange={(e) => setStdin(e.target.value)} rows={8} className="font-mono" />
            </TabsContent>
