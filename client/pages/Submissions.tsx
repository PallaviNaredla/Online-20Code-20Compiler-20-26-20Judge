...
export default function Submissions() {
  ...
  useEffect(() => {
    apiListSubmissions().then(setItems).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <Layout>
      ...
      {!loading && items.length === 0 && <p className="text-muted-foreground">No submissions yet.</p>}
      <div className="grid gap-3">
        {items.map((s) => (
          <div key={s.id} className="border rounded-lg p-4 bg-card">
            ...
            {s.runSummary && (
              <div className="mt-2 text-sm text-muted-foreground">Summary: {s.runSummary.passed}/{s.runSummary.total} passed · {s.runSummary.durationMs}ms</div>
            )}
            <pre className="mt-3 bg-muted p-2 rounded text-xs overflow-auto max-h-48">{s.code}</pre>
          </div>
        ))}
      </div>
    </Layout>
